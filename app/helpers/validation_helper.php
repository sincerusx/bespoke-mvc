...
if( !function_exists('is_valid_age')){
	function is_valid_age($day, $month, $year)
	{
		...
		for($i = 0; $i<count($rules); $i++){
			...
			switch($requirement){
				...
				case "valid_email":
					...
					if(isset($fields[ $field_name ]) && !empty($fields[ $field_name ]) && !preg_match($regexp,
																									  $fields[ $field_name ])
					){
						$errors[ $field_name ] = $error_message;
					}
				...
				case "valid_date":
					...
					if( !is_valid_date($fields[ $field_name ],
									   $fields[ $field_name2 ],
									   $fields[ $field_name3 ],
									   $is_later_date)
					){
				...
				case "reg_exp":
					$reg_exp_str = str_replace("�", ",", $field_name2);
